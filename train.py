...
parser.add_argument('--save_dir', type=str, default='./saved_models_aggcn', help='Root dir for saving models.')
...
train_batch = DataLoader(opt['data_dir'] + '/train_new.json', opt['batch_size'], opt, vocab, evaluation=False)
...
file_logger = helper.FileLogger(model_save_dir + '/' + opt['log'], header="# epoch\ttrain_loss")
...
for epoch in range(1, opt['num_epoch']):
    ...
    if epoch == 1 or train_loss < max(train_loss_history):
